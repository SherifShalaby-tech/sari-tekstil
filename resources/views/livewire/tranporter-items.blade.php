                                        ...
                                        <tr class="{{ $row_background_color }}">
                                            ...
                                            <td>
                                                @foreach ($car->car_contents as $content)
                                                    {{ $content->nationality->name ?? '' }}-{{ $content->percentage ?? '0' }}%
                                                    <br>
                                                @endforeach
                                            </td>
                                            ...
                                            <td class="justify-content-between d-flex">
                                                @if ($car->used_cart == 'used')
                                                    <button class="btn btn-danger"
                                                        {{ $car->used_cart == 'sewing' ? 'disabled' : '' }}
                                                        wire:click="move({{ $car->id }})">@lang('lang.move')</button>
                                                @elseif($car->used_cart == 'not_used')
                                                    <button class="btn btn-success"
                                                        {{ $car->used_cart == 'sewing' ? 'disabled' : '' }}
                                                        wire:click="useCar({{ $car->id }})">@lang('lang.use')</button>
                                                @endif
                                                &nbsp;
                                                @if ($car->used_cart == 'sewing')
                                                    <button class="btn btn-warning"
                                                        wire:click="sewing({{ $car->id }},'not_used')"><i
                                                            class="fa fa-wrench"></i></button>
                                                @else
                                                    <button class="btn btn-outline-warning"
                                                        {{ $car->used_cart == 'used' ? 'disabled' : '' }}
                                                        wire:click="sewing({{ $car->id }},'sewing')"><i
                                                            class="fa fa-wrench"></i></button>
                                                @endif
                                                &nbsp;
